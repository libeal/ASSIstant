...
linux_agent_backup_runtime() {
    ...
    linux_agent_log_event "runtime_backup_created" "$(jq -cn --arg name "${output_name}" --arg sha256 "${sha256}" --argjson size_bytes "${size_bytes}" '{name:$name, sha256:$sha256, size_bytes:$size_bytes}')"
