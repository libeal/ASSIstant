    ...
    jq -c --arg stage "${stage}" --argjson limit "${limit}" '
        ...
        if ($stage == "command_started" or $stage == "command_finished") then
        ...
        elif $stage == "request_context_built" then
            {
                ...
                candidate_skill_count:(if (.skills.candidates? | type) == "array" then (.skills.candidates | length) else 0 end),
                candidate_skills:[.skills.candidates[]?.name],
                loaded_skill_file_count:(if (.skills.loaded? | type) == "array" then (.skills.loaded | length) else 0 end),
                loaded_skills:([.skills.loaded[]?.name] | unique),
