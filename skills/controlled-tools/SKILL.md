...
| `file-patch.sh` | `path:string`、`find:string`（非空）、`expected_count:integer`（>=1） | `replacement:string`（空串）、`apply:boolean`（true）、`backup:boolean`（true）、`max_file_bytes:integer`（2 MiB；正数）。实际匹配数必须严格等于 `expected_count` |
