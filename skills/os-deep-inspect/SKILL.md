...
调用形式为 `bash scripts/<name>.sh '<json-object>'`，唯一位置参数必须是 JSON object。stdout 只输出一个 JSON object；以 `ok`、`error` 判断业务结果。所有限制字段都是边界，不得依赖脚本截断非法值来扩大采集范围。

## 参数契约

| Script | 必填字段 | 可选字段（类型；默认；约束） |
| --- | --- | --- |
| `os-snapshot.sh` | 无 | `top_n:integer`（10；1..50）、`journal_lines:integer`（30；1..100） |
| `net-inspect.sh` | 无 | `port:integer`（空；1..65535）、`protocol:string`（`all`；`all/tcp/udp`）、`state:string`（空）、`limit:integer`（80；1..300）、`include_process:boolean`（false） |
| `fd-inspect.sh` | 无 | `pid:integer`（空；正整数）、`pattern:string`（空）、`limit:integer`（80；1..300）。没有 `lsof` 时必须提供 pid |
| `journal-inspect.sh` | 无 | `unit:string`（空；安全 unit 字符）、`priority:string|integer`（空；0..7 或合法名称）、`since:string`、`until:string`（均为空）、`boot:boolean|integer`（空；true 或 boot 偏移）、`lines:integer`（80；1..300）、`grep:string`（空） |

示例：`bash scripts/journal-inspect.sh '{"unit":"sshd.service","priority":"err","since":"-1 hour","lines":50}'`。
