...
| `config-backup.sh` | `path:string` | `backup_root:string`（`/tmp/linux-agent-backups`）；输出带源文件 SHA-256 的备份凭据 |
| `safe-log-cleanup.sh` | `path:string` | `max_size_mb:integer`（100；>=0）、`dry_run:boolean`（true）、`backup_archive:string`、`backup_sha256:string`（真实截断时必填且必须匹配目标快照）。只接受受控日志路径和普通非符号链接文件 |
...
清理磁盘或日志时，先调用 `disk-hotspots.sh` 和 `log-cleanup-plan.sh`。真实清理前必须先调用 `config-backup.sh`，再把它返回的 `archive` 与 `source_sha256` 传给 `safe-log-cleanup.sh`；缺少或不匹配时执行器和脚本都会拒绝截断。
