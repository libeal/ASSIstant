export const CONFIG_GROUPS = [
  {
    ...
    fields: [
      ...
      { key: "provider_resilience.enabled", label: "Provider 韧性", type: "boolean", onEffect: "启用有界重试、熔断和显式备用 Provider。", offEffect: "每次仅调用主 Provider 一次。" },
      { key: "provider_resilience.max_attempts", label: "max_attempts", type: "number", min: 1, max: 5, comment: "单个 Provider 对可重试故障的总尝试次数。" },
      { key: "provider_resilience.backoff_initial_ms", label: "backoff_initial_ms", type: "number", min: 0, max: 60000, comment: "第一次重试前的等待毫秒数。" },
      { key: "provider_resilience.backoff_max_ms", label: "backoff_max_ms", type: "number", min: 0, max: 60000, comment: "指数退避的最长等待毫秒数。" },
      { key: "provider_resilience.circuit_failure_threshold", label: "circuit_failure_threshold", type: "number", min: 1, max: 100, comment: "连续可重试故障达到该次数后开启熔断。" },
      { key: "provider_resilience.circuit_open_sec", label: "circuit_open_sec", type: "number", min: 1, max: 86400, comment: "熔断开启后等待半开探测的秒数。" },
...
export const CONFIG_READONLY_FIELDS = [
  ...
  { key: "provider_resilience.failover_count", label: "provider failover", comment: "备用 Provider 数量；端点和密钥环境变量需在 config.json 中显式配置。" },
